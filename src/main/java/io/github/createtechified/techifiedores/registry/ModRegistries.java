...
public class ModRegistries {
    ...
    public static void register(IEventBus modBus) {
        for (MaterialDef mat : ModMaterials.MATERIALS) {
            registerMaterial(mat, mat.harvestLevel());
        }
    ...
    private static void registerMaterial(MaterialDef mat, int harvest) {
        // Blocks
        if (mat.hasBlock()) registerBlock(harvest, mat, "block", Blocks.IRON_BLOCK);
        if (mat.hasRawOreBlock()) registerBlock(harvest, mat, "raw_ore_block", Blocks.RAW_IRON_BLOCK);

        // Ores
        if (mat.hasOverworldOre()) registerBlock(harvest, mat, "ore", Blocks.STONE);
        if (mat.hasNetherOre()) registerBlock(harvest, mat, "nether_ore", Blocks.NETHERRACK);
        if (mat.hasEndOre()) registerBlock(harvest, mat, "end_ore", Blocks.END_STONE);
    ...
    private static void registerBlock(int harvestLevel, MaterialDef mat, String form, Block template) {
        String name = mat.name() + "_" + form;
        RegistryObject<Block> block = BLOCKS.register(name, () -> new TintableMaterialBlock(mat, form, harvestLevel, BlockBehaviour.Properties.copy(template).requiresCorrectToolForDrops()));
